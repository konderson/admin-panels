                ...
                <div class="span12">
                    <div class="widget-box">
                        ...
                        <div class="widget-content nopadding">
                            <form class="form-horizontal" method="post" action="{{url('/admin/edit-category/'.$categoryDetails->id)}}" name="add_validate" id="add_validate" novalidate="novalidate">
                                {{csrf_field()}}

                                <div class="control-group">
                                    <label class="control-label">Название категории</label>
                                    <div class="controls">
                                        <input type="text" name="category-name" id="name" value="{{$categoryDetails->name}}">
                                    </div>
                                </div>
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                        <select name="parent_id">
                                            ...
                                            @foreach($levels as $level)
                                                <option value="{{$level->id}}" @if($level->id==$categoryDetails->id) selected> @endif {{$level->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
